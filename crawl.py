...
class WebCrawler:
    def __init__(self, query, max_pages=500, max_depth=3, concurrent_requests=20, num_threads=4):
        ...
        self.visited = set()
        self.url_queue = []  # Priority queue
        self.domain_counts = defaultdict(int)  # Track domain visit counts
        self.domain_last_visit = defaultdict(float)  # Track last visit time per domain
...
def main():
    ...
    crawler = WebCrawler(
        ...
        max_pages=5000, 
        max_depth=3, 
        concurrent_requests=40,  # Increased for multithreading
        num_threads=10
    )
    crawler.crawl()
